...
def convert_smartdok_rue_record_to_dict(raw_record: dict = None) -> dict:
    ...
    return NotImplementedError(
        f"this isent implemented yet - raw_record={raw_record} was not converted to a dict"
    )
...
def convert_smartdok_qd_record_to_dict(raw_record: dict = None) -> dict:
    ...
    raw_category_dict = {
        "type": raw_record["Values"][0]["Type"]
        if raw_record["Values"][0]["Type"]
        else None,
        "id": raw_record["Values"][0]["Values"][0]
        if raw_record["Values"][0]["Values"]
        else None,
    }
...
def convert_qd_category_dict_to_text(
    raw_record: dict = None, category_dict: dict = None
) -> str:
    """
...
def convert_date_to_smartdok_date_format(days_back: int = None) -> (str, str):
    ...
    days_back_date = (current_datetime - datetime.timedelta(days=days_back)).replace(
        microsecond=0
    ).isoformat() + "+01:00"
...
def verify_init_params(
    username: str = None, password: str = None, user_agent: str = None
) -> bool:
    """
    ...
    if user_agent is None:
        ...
        logging.debug(
            f"verify_init_params - username={username}, password={password}, user_agent={user_agent}"
        )
